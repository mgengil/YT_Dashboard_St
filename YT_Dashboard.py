...
@author     : kenne
@referance  : https://github.com/PlayingNumbers/YT_Dashboard_st/blob/main/Ken_Dashboard.py
"""
...
def style_negative(v, props=''):
    """ Style negative values in dataftame"""
    try:
...
# @st.cache  means that when we refresh the page it wont call the function everytime.
# Function have been saved in cache.
@st.cache
def load_data():
    """ Loads in 4 dataframes and does light feature engineering"""
    df_agg = pd.read_csv('Aggregated_Metrics_By_Video.csv').iloc[1:, :] # first row include total value of data
...
views_days = pd.pivot_table(df_time_diff_yr,index= 'days_published',values ='Views', aggfunc = [np.mean,np.median,lambda x: np.percentile(x, 80)]).reset_index()
views_days.columns = ['days_published','mean_views','median_views','80pct_views']
views_days['20pct_views'] = pd.pivot_table(df_time_diff_yr,index= 'days_published',values ='Views', aggfunc = [lambda x: np.percentile(x, 20)]).reset_index()['<lambda>']
views_days = views_days[views_days['days_published'].between(0,30)]
views_cumulative = views_days.loc[:,['days_published','median_views','80pct_views','20pct_views']]
views_cumulative.loc[:,['median_views','80pct_views','20pct_views']] = views_cumulative.loc[:,['median_views','80pct_views','20pct_views']].cumsum()

## What metrics will be revelant?
##Difference between baseline
## Percent change by video


#build dashboard
# adding a sidebar on page
...
if add_sidebar == 'Aggregate Metrics':
    ...
    # colums help us to show the data in a row. otherwise it will be showing in a column
    col1,col2,col3,col4,col5 = st.columns(5)
    columns = [col1,col2,col3,col4,col5]

    #to show all in columns
    count = 0
    for i in metric_median6mo.index:
        with columns[count]:
            delta = (metric_median6mo[i] - metric_median12mo[i]) / metric_median12mo[i] # shows us increasing or decreasing value
            # showing the value on page
    ...
    df_agg_numeric_lst = df_agg_diff_final.median().index.tolist() # take only numeric columns
    df_to_pct = {}
    for i in df_agg_numeric_lst:
        df_to_pct[i] = '{:.1%}'.format

    #shows us dataframe value on page
...
if add_sidebar == 'Individual Video Analysis':
    ...
    # Shows us selecting box on page
    video_select = st.selectbox('Pick a video: ',videos)

    #find to show the video
